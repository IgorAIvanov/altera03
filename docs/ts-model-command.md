...
- **Нужен либо `module`, либо `handlerKey`** — генератор упадёт, если нет ни того,
  ни другого. `module` — команда самого приложения, файл рядом с моделью.
  `handlerKey` — готовый рантайм-хендлер ядра, например
  `{ "handlerKey": "runtime.printPdf" }`: приложение не знает путей внутри
  `server/`, ключ резолвится картой `RUNTIME_HANDLERS` в `model-registry.ts`.
  Список доступных ключей — там же.
