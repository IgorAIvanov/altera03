...
- **Фокус после закрытия.** Окно — не нативный `<dialog>`, а оверлей
  `picker-host`: при закрытии элемент с фокусом просто исчезает из DOM, и фокус
  падал на `body` — после выбора по F4 поле оставалось без него. Хост
  запоминает, где стоял фокус при открытии (`deepActiveElement()` из
  `client/ui-kit/focus-order.ts` — сквозь shadow root до самого `input`), и
  возвращает его туда, **но только если фокус пропал**: открывавший мог уже сам
  поставить его, куда нужно. `<ui-picker>` сверх того возвращает фокус в своё
  поле — при клике мышью запомненным оказалась бы лупа. То же правило — в
  `confirm-host` (`bus.confirm` / `bus.choose`).
