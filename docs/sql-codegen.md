...
Документ-підстава — звичайна річ в обліку: повернення посилається на реалізацію,
коригування податкової накладної — на саму накладну, акт звірки — на договір.
Оголошується вона тим самим `x-ref`, і нічого додаткового казати не треба:
...
shipment: Type.Optional(Type.Union([Type.Object({}), Type.Null()])),
```

Тип цілі генератор бере з її `manifest.json`, і на документі вмикає дві поправки
— рівно ті, якими документ відрізняється від довідника:

- **join за `document_id`, а не за `id`.** Документ не має власної identity:
  первинний ключ його таблиці — це посилання на шапку.
- **подання береться з шапки**, бо номера, дати й представлення в таблиці моделі
  немає. Тому join **подвійний**:
  ```sql
...
**Подання за умовчанням — `presentation`, з падінням на номер:**
`coalesce(nullif(d_shipment.presentation, ''), d_shipment.number)`. Це та сама
денормалізована колонка, якою підписаний рядок у журналі документів, тобто
документ уже названий так, як його називають люди. Заповнює її **необов'язковий**
хук `app.<model>_denormalize` — документ, який його не має, показувався б порожнім
рядком, і в пікері це невідрізненно від зламаного поля; звідси падіння на номер.

Складати «№1 від 22.07.2026» у генераторі не можна: слово «від» — це текст мовою,
а сервер тексту не перекладає (він його називає маркером, а маркери в **полях
даних** не розгортаються — лише в `messages`). Хочете саме такий підпис — складіть
його у своєму `_denormalize`, мовою свого застосунку.

**`display` вибирає колонку явно**, і працює по обидва боки: `display: "number"`
або `"doc_date"` — колонка шапки, будь-яка інша — власний реквізит документа
(тоді другого join немає). Ключ вкладеного об'єкта — camelCase від колонки, тож
`display: "doc_date"` дає `{ id, docDate }`, і саме це ім'я має стояти у формі:
`<ui-picker display-field="docDate">`. Для `presentation` — `display-field="presentation"`.

Той самий резолв діє й у **ссылочному фільтрі звіту**: інакше та сама анотація
працювала б у формі й падала у фільтрі.

`sortable` працює по будь-якому поданню, а `searchable` — лише по текстовому
(`number`, `presentation`, `description`): пошук іде `ilike`, і по даті чи сумі
це помилка типу. Відмова приходить **на генерації**: сама по собі вона вилізла б
на першому наборі символів у полі пошуку, бо тіло `list` — plpgsql, і
`create function` його не перевіряє.

### Ссылка на БУДЬ-ЯКИЙ документ (`entity: "document"`)

Названа `model` годиться, доки писар один. Регістр відомостей на те й регістр, що
...
параметри амортизації — введення й модернізація. Рядок, записаний ДРУГИМ
документом, у join через таблицю однієї названої моделі не знаходиться — і колонка
«Документ» мовчки порожня, тобто екран каже неправду: «цей рядок завели руками».

Ознака саме цього випадку: FK у базі стоїть, `select` по таблиці показує
`document_id`, а список моделі показує порожньо. Ані генерація, ані `check`, ані
публікація нічого не кажуть — join валідний, він просто не про той документ.

Тому ціль можна назвати не моделлю, а сутністю:
...
Join у шапку тоді **прямий** (у названого документа їх два — своя таблиця плюс
шапка), а другим іде тип:

```sql
left join app.document r_document on r_document.id = t.document_id
left join app.document_type dt_document on dt_document.id = r_document.document_type_id
```

Подання те саме, що й у названого документа (`presentation` з падінням на номер),
але з одного аліаса. А вкладений об'єкт несе ще й **код типу**:

```json
"document": { "id": "1419", "presentation": "Введення в експлуатацію №ДМВЕ-000001 …", "typeCode": "fixed_asset_commissioning" }
```

`typeCode` — це КЛЮЧ МОДЕЛІ, тобто те, з чого виводиться маршрут в'ю
(`viewRoute(typeCode)`), і без нього з рядка не було дороги в документ: рядок
знав, ЯКИЙ документ його поставив, і не знав, ЯКОГО той типу. Перше, що робить
людина, побачивши колонку з реєстратором, — намагається його відкрити.

Тільки в `entity`, і не з міркувань економії: ссылка, що назвала `model`, і так
знає модель — код у неї сталий, він у самому оголошенні. Ціль називають **рівно одним способом** — `model` або
`entity`; обидва разом і жодного — помилка генерації, бо і те, й те мовчки дало б
join не про той документ.

Ціна рішення одна, і вона в самій постановці: показати можна **лише колонку
спільної шапки** (`organization_id`, `number`, `doc_date`, `total`,
`presentation`, `description`, `is_posted`, `is_deleted`). Власних реквізитів у
«будь-якого документа» немає за визначенням — не знаючи моделі, про нього відомо
рівно те, що в шапці; `display` поза цим переліком відхиляється на генерації.

Пікера над журналом документів це не дає: `<ui-picker url="…">` іменує **маршрут
в'ю**, а журнал — екран застосунку, і фреймворк його за нього не оголосить.
Для реєстратора регістра це не заважає: колонку заповнює проведення, а не руки.
...
```
post   → doc_post_begin (зносить проводки)
       → app.<model>_unpost_records(user_id, document_id)    ← НЕОБОВ'ЯЗКОВА: зносить рухи
       → app.<model>_post_entries(user_id, document_id)      ← рукописна
       → doc_post_finish
unpost → doc_unpost (зносить проводки)
       → app.<model>_unpost_records(user_id, document_id)    ← та сама
```
...
**`_unpost_records` — пара до неї, і потрібна лише документу, який пише рухи КРІМ
проводок.** `doc_unpost` чистить `app.journal_entry` і більше нічого: про рядок,
покладений у ЧУЖУ таблицю — періодичний реєстр цін, склад, ПДВ, — ядро не знає
нічого. Без цієї половини ціни розпровединого документа далі діяли б у накладних,
а екран показував би документ непроведеним.
...
- **кличеться на ОБОХ шляхах, і на проведенні це важливіше.** `doc_post_begin`
  повторному проведенню не відмовляє — він зносить проводки й дає провести
  наново; без виклику перед `_post_entries` рядки в чужій таблиці подвоювалися б
  із кожним перепроведенням. Тобто правило «перепроведення переписує начисто»,
  яке ядро тримає для проводок, поширене й на оголошені рухи;
- **вмикається створенням функції** — так само, як `app.doc_before_write`. Немає
  її — немає й виклику, тож наявні документи не переписуються;
- **те саме ім'я з іншим підписом валить розпроведення**, а не пропускається:
  мовчазний пропуск тут гірший за відмову — застосунок був би певен, що рухи
  зняті, а вони лишилися б діяти;
- **тригер на `app.document` замість цього ставити не треба.** Він працює, і в
  цьому пастка: на ту саму таблицю вішає тригери й ядро, порядок спрацювання ніде
  не записаний, а коли вони почнуть заважати один одному, дізнаєшся з даних, а не
  з помилки.

Читання чужого реєстру звичайно генерується (`periodic`), тобто дописати умову
«лише проведений документ» у нього нікуди — саме через це обхід і не складався.
...
### Періодичні дані — `periodic` у манифесті

Курси валют, ціни, ставки податків, облікова політика — це ключ, дата й
значення, і механіка в них однакова. Тому вона оголошується:

```jsonc
"periodic": { "key": ["currencyId"], "period": "period" }
```

Ключ і період називаються **полями схеми** (як `numbering.field`), а не
колонками; поле, якого немає в `ItemSchema`, — помилка генерації, а не тихий
зріз по не тому ключу. До звичайного CRUD додається три команди й один індекс:

| Команда | Що робить |
|---|---|
| `<model>_at` | зріз останнього: по рядку на ключ, найсвіжіший із тих, що не пізніші за `onDate` (без дати — сьогодні). Ключ у payload звужує вибірку; коли рядок один, він приходить ще й в `item` |
| `<model>_history` | усі значення ключа, свіжі зверху; необов'язкові `dateFrom`/`dateTo` |
| `<model>_set` | перезапис значення **на дату**: `merge` за природним ключем (ключ + період), а не за `id` — імпорт курсів ідентифікаторів рядків не знає |

Права цим командам виводяться з того самого блока (`at`/`history` — `view`,
`set` — `edit`), і рантайм маршрутизує їх без оголошення в `commands.sql`:
повторювати в манифесті те, що вже там сказано, довелося б лише для того, щоб
забути й отримати «команди немає» на першому виклику.

**Унікальний індекс `(ключ…, період desc)` кладе сам генератор** — це єдиний
випадок, коли в згенерований файл потрапляє DDL, і він свідомий. Індекс
виводиться з того самого оголошення, що й функції: унікальність тримає `_set`
(двох значень на одну дату для одного ключа не буває), а `desc` — те, що
`distinct on (ключ) order by ключ, період desc` бере індекс, а не сканує
таблицю; btree читається цілком уперед або цілком назад, тож напрямок мусить
збігатися. Тому в `struc.sql` цього індексу писати не треба.

Еталон — `app/data/currency_rate/`.
...
**Ключ, оголошений у фільтрі, мусить збігатися з ключем эха** — і збігаються вони
не самі собою. Эхо бере ключ із подання цілі, тобто з **першого поля з
`x-lookup`**: у більшості довідників це `name`, але в складі, одиниці виміру й
банківському рахунку — `code`. Эхо приходить першим у `v_filters || …`, тож при
розбіжності воно ЗАТИРАЄ підпис, покладений формою: відбір діє, числа правильні,
а пікер у панелі порожній. Схема при цьому валідна, SQL зелений, і шукати причину
йдуть у форму й у пікер — тобто куди завгодно, крім згенерованої обгортки.

Тому `sql:gen` відмовляє й називає обидві половини:
...
Оголошення без підпису (`{ id }`) суперечності не містить і проходить: эхо просто
дописує ключ, якого форма не обіцяла.
