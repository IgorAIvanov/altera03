...
| `or exists (…)` у пошуку | `x-search-via` на корені `ItemSchema` |
...
- **`x-search-via: { model, field, fk? }`** — пошук крізь ПІДПОРЯДКОВАНУ модель:
  позиція знаходиться за штрихкодом, артикулом постачальника чи синонімом, що
  лежать в окремій таблиці (у позиції їх кілька). Анотація стоїть на **корені**
  `ItemSchema` власника, а не на полі — поля, за яким шукають, у власника немає:

  ```ts
  Type.Object({ … }, { "x-search-via": { model: "nomenclature_barcode", field: "barcode" } })
  ```

  `field` і `fk` — ключі схеми підпорядкованої моделі; `fk` за умовчанням — її
  єдине поле з `x-ref` на власника (кілька або жодного — відмова з переліком).
  Кілька таблиць — масивом. У `_list` і `_lookup` генератор дописує
  `or exists (select 1 from … where <fk> = t.id and <field> ilike …)`: позиція
  приходить ОДИН раз, хоч би скільки її кодів збіглося (join множив би рядки), а
  позначений на видалення код (`isDeleted` у підпорядкованій схемі) не знаходить
  нічого. Власні колонки шукаються як і раніше — фоллбек «усі строкові» не
  зникає від того, що з'явилася анотація. Помилки схеми (поле не текстове, чуже
  `x-ref`, ціль — документ) — на генерації. **Колонці `fk` потрібен індекс**:
  `exists` пробує її на кожен рядок власника.
...
### Ссылка на документ як зв'язок (`app.document_link`)

Кожна ссылка **документа** на документ — названою `model` чи `entity: "document"`,
у шапці чи в рядку табличної частини — стає ребром дерева «Пов'язані документи».
Оголошувати для цього нічого не треба: `sql:assemble` виводить зі схем моделей
`sql.json` представлення `app.document_link` (`from_id`, `to_id`, `model`, `field`)
з гілкою `union all` на ребро і ставить його останньою секцією функцій
(`_generated/document-link.sql`). Резолв колонки той самий, що в CRUD:
`x-ref.fk`, потім `x-db-col`, потім ім'я поля.

Відмова — на самому посиланні, якщо воно технічне і зв'язком не є:

```ts
"x-ref": { entity: "document", as: "noteDocument", related: false },
```

Три речі, які варто знати:

- **Ссылки регістрів і довідників ребрами не стають** — лише реквізити документа.
  Реєстратор у регістрі — це рух документа, а не зв'язок двох документів.
- **Голий `bigint` без `x-ref` зв'язком не стає**, хоча FK у DDL стоїть: генератор
  читає схему, а не базу. Колонка, яку форма добирає сама, з дерева випадає мовчки.
- **Заглушка ядра скидає представлення на кожній публікації** (`@core/document_core`,
  struc) — щоб міграція, яка видаляє чи змінює колонку-посилання, не впиралася в
  залежне представлення. Справжнє ставиться після міграцій. Звідси наслідок: SQL
  застосунку на `app.document_link` у структурі й міграціях покладатися не може.

Публікація після накочування кличе `app.document_link_check()` і друкує `⚠`, якщо
колонка ребра без індексу (обхід дерева перегляне таблицю цілком) або FK документа на
документ ребром не став (голий `bigint` без `x-ref`). Це попередження, а не відмова.

План і решта механізму — [`related-documents-plan.md`](related-documents-plan.md).
