...
Через це вона не входить і в реєстр застосунку — з двома наслідками, які видно
лише збоку. **Зовнішньому агенту** її оголошує ядро окремо
(`server/modules/agent/core-agent-tools.ts`, лише `list` і `get`): перелік
інструментів збирається з манифестів, і без цього агент бачив би документ без
жодного прикріпленого файлу. **На екрані груп** її треба дописати рядком у
перелік моделей — інакше право `attachment:view` видається лише зіркою.
