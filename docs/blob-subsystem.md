...
Секрет підпису — `BLOB_TOKEN_SECRET`, інакше `JWT_SECRET`. Якщо не задано
жодного, генерується разовий ключ процесу (dev): після рестарту старі
посилання перестають діяти, і в лог іде попередження.
