...
const TOOLS = [
  ...
  },
  {
    ...
    description:
      "Схеми payload-ів команд названих моделей (JSON Schema) і ОГОЛОШЕНІ ОБМЕЖЕННЯ — те, " +
      "чого застосунок робити не стане. Кілька моделей одним викликом — так дешевше, ніж " +
      "по одній. Викликати перед altera_call, якщо склад полів невідомий: вгадувати поля " +
      "не треба, вони описані точно. Правила читати перед тим, як РАДИТИ порядок дій: " +
      "поле в схемі буває, а команда його відбиває — і тоді людина впирається в це не " +
      "зараз, а через місяць роботи.",
    inputSchema: {
      type: "object",
      properties: {
        models: {
  ...
  },
  {
    ...
    description:
      ...
      "людині як є. " +
      "ШУКАЙ, А НЕ ЧИТАЙ ЦІЛКОМ: `list` бере `search`, `pageSize` (не `limit` — незнайомий ключ " +
      "мовчки ігнорується) і відбори — щоб знайти один рядок, " +
