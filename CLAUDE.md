...
своя кнопка збереження вішається на `save`/`trySave`, а не на `saveItem`. Табличні частини не
покриті — там перевірка досі тільки в БД. Деталі — [`docs/ui-form-validation.md`](docs/ui-form-validation.md);
