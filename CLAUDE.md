...
читався б як «поміряли й вийшло нічого». **Кількість зберігається по боках проводки**
(`quantity_debit` / `quantity_credit` у `app.journal_entry`) — це небалансовий вимір, як у
джерелі моделі: комплектація списує 6 корпусів і оприбутковує 2 комплекти однією
кореспонденцією, і нерівність тут зміст операції, а не помилка. Складна проводка — це
кілька таких рядків, кількість головного боку пишеться один раз; окремого механізму групи
немає навмисно (сверяти в ній нема чого — сума голови дорівнює сумі частин за побудовою).
У `doc_entry_add` два шляхи: `p_quantity` — одне число на обидва кількісні боки, строго, як
було; `p_quantities` (`{"debit": 2, "credit": 6}`) — по боках, і перелік тут ВИСЛОВЛЮЄ
НАМІР: відсутній у ньому кількісний бік лишається порожнім законно, а `{}` — це переоцінка
(сума без кількості). І **розріз усе ще один** (`p_dimension_code`), тобто «скільки чого й
