...
mcp/                        # MCP-обгортка над HTTP-API (@altera/mcp)
  ...
  README.md                 # запис у конфізі хоста, кілька баз, токен «тільки читання»
                            #   у реєстр поки не публікується (D6): версія протоколу —
                            #   клопіт обгортки, і поки MCP рухається, вона лишається тут
...
інакше (заміряно 127 байтів на модель), тож його тримають цілком, а схему беруть
тоді, коли справді збираються викликати. Перелік **фільтрується правами
