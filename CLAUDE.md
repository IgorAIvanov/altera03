...
**Опис інструментів** — `GET /api/agent/tools` віддає `{model, command, input}`,
де `input` це JSON Schema payload-а, згенерована з TypeBox-схем моделей
...
`{type: "object"}` і не знав ані складу полів, ані типів. Перелік **фільтрується
правами користувача** — не як захист (він нижче, у рантаймі), а щоб агент не
витрачав крок на команду, яка йому відмовить.
