...
`server/modules/agent/` — це **дві різні речі під одним іменем**, і плутати їх
шкідливо. `agent.service.ts` (114 рядків) — диспетчер без жодного LLM: звіряє
модель і команду з білими списками (`manifest.agent`) і кличе рантайм від імені
користувача. `agent-llm.service.ts` (893 рядки) — клієнт OpenAI Responses API з
власним роутером і tool-циклом. **Обидва ендпойнти не викликає ніхто** — ні
екрана, ні клієнтського коду, — але їдуть вони в кожен опублікований пакет.

Напрям обраний: своєї «розумної» частини не пишемо, працює **готовий зовнішній
агент від імені реального користувача**. Диспетчер лишається основою, LLM-сервіс
іде під ніж. Найважче для такої задачі вже зроблено не заради неї: право
рахується на кожній команді (fail-closed), `Authorization: Bearer` уже
приймається, аудит пише виконавця, а `manifest.agent` дає другі ворота, вужчі за
права.

Зроблено два кроки. **Персональний токен** (`app.access_token`, `@core/access`)
— облікові дані для машини: `Authorization: Bearer`, права не несе (їх рахує
`access_can` як завжди), видається ЛИШЕ з браузера по сесії, у базі лежить хеш.
Керування — на `/api/auth/tokens`, поза правами моделей: токен це «моє», як
власний пароль, а модель вимагала б прав на неї в кожного користувача.
