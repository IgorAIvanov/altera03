...
**Аудит команд працює за замовчуванням** — і саме для того, заради чого журнал змін
існує: стандартні команди, які запис МІНЯЮТЬ (`save`, `delete`, `undelete`, `post`,
`unpost`), пишуться без жодного оголошення. Доти політику вмикав поіменний список у
`manifest.json`, і з дванадцяти моделей його оголосила рівно одна: позначка на видалення
не лишала сліду ніде, тобто «хто прибрав запис» не знаходилося саме там, де це шукають.
Умовчання тут не послаблення, а те саме міркування, що з правом `undelete`: команду видає
генератор усім моделям одразу, а руками її дописують у кожен манифест — забути можна лише
в один бік, і мовчки.

Читання (`list`, `get`, `lookup`) в умовчання не входить: воно роздуло б журнал так, що
змін у ньому не знайти. Нестандартна команда (`copy`, `moveToGroup`, будь-яка TS-команда)
теж — журнал не знає, чи щось міняє команда з довільним іменем, тож назвати себе вона
мусить сама. Оголошення в манифесті сильніше за умовчання в обидва боки:

```json
"audit": { "commands": ["save", "delete", "undelete", "copy"] }   // рівно ці
"audit": true                                                      // усі, включно з читанням
"audit": false                                                     // не писати нічого
```
...
поставки `app/.solution.json` із SHA-256 кожного файлу; `deno task solution:status` звіряє
з ним диск. Три стани: **на підтримці** (дерево збігається — оновлення автоматичне),
**знято з підтримки** (щось правили — `app/` не чіпається, пакет лягає в `app.incoming/`
разом зі списком того, чого торкнулися обидві сторони), **невідомо** (манифесту немає —
звірити нема з чим, потрібен `--force`). Оголошувати нічого не треба: правка одного файлу
сама переводить установку в ручний режим і цим захищає саму себе.

**Оновлює окрема команда** — `deno task solution:update -- <пакет>`: розпакування →
`sql:registry` → `sql:deploy --yes` → `build:front`, далі перезапуск за тим, хто керує
сервісом. Кнопки установки в застосунку немає навмисно: **той, кого заміняють, не має
заміняти себе сам** — вона могла переписати власний `app/`, а перезапуститися після цього
ні, і заразом вимагала дати серверу `--allow-write` і `--allow-run`. Тепер ці дозволи
разові й належать оновлювачу, а `server/modules/solution/` лишився **тільки на читання**:
віддає стан підтримки, який показує стартова вкладка.

**У контейнері образ несе ФРЕЙМВОРК, а рішення живе на томі** (`Dockerfile` і профіль
`prod` у `docker-compose.yml` шаблону). Звідси два незалежні шляхи оновлення: нове рішення
— розпакувати на том і зібрати, образ той самий; новий фреймворк — замінити образ, том той
самий. Ціна одна й головна: **заміна образу вимагає перезбірки**, бо `dist/` і
`_sqlpackage/` на томі зібрані попередньою версією (у першому вбитий клієнтський код
фреймворку, у другому — SQL ядра). Розбіжність показує `solution:status` — манифест несе
піни, якими артефакти зібрані. Обслуговуючий контейнер монтує том **лише на читання**, тож
незмінність рішення тут не домовленість, а властивість; писати може лише разова служба
`update`.

**Оновлення встановленого застосунку** — [`docs/update-guide.md`](docs/update-guide.md):
підтримка як стан, окрема команда оновлення, три розгортання (приватний сервер, контейнер,
Deno Deploy) і Git як механізм злиття для зміненого рішення (гілка `vendor` + `git merge` —
окремого механізму не треба). Ключове: версію фреймворку називає рішення, патчі вільні,
мінори — лише разом із рішенням; `deno.lock` у дереві обов'язковий.
