...
Плейсхолдери: `{ORG}` (префікс організації), `{TYPE}` (префікс типу документа),
`{YYYY}`/`{YY}`, `{MM}`, `{N…}` (ширина = довжина плейсхолдера). Манифест дає
**умовчання**: сід іде `on conflict do nothing`, далі правило живе на admin-екрані,
і деплой правку не затирає — той самий шлях, що в шаблонів друку. Зворотний бік
того самого: змінений у манифесті шаблон на вже налаштованій базі **не
застосується** — його міняють на екрані.

**Шаблон задає ВИГЛЯД номера, `period` — коли лічильник починається спочатку
(`none` | `year` | `month`), і одне з одного не виводиться.** Виводити період із
шаблона («є `{YY}` — значить щороку») виглядає елегантно і хибно по суті:
нумерація в межах року потрібна майже всім, а рік у самому номері друкують далеко
не всі — виведення змусило б їх вибирати між форматом номера й обліковим правилом.
Область лічильника — так само факти, а не шаблон: організація ділить лічильник
завжди, коли вона в записі є, незалежно від `{ORG}` у шаблоні (як і в
`uq_document_number`, куди `organization_id` входить безумовно).

Унікальність тримає не форма номера, а **індекс по даті документа**:
`uq_document_number` включає `date_trunc('year', doc_date)`. Окремого стовпчика з
періодом немає навмисно — дата вже є, а копія розходилася б із нею. Звідси й межа:
рік у виразі індексу зашитий (індекс один на таблицю, а `period` — рядок
конфігурації), тому `month` вимагає `{MM}` у шаблоні — інакше січневий і лютневий
№1 зіткнулися б. Це перевіряє `app.numerator_validate`. Період береться **з дати
документа**, тож довідник лишається на `none`: дати в нього немає.

**Дві стратегії лічильника**, поле `strategy`. `counter` (умовчання) — лічильник у
рядку звичайної таблиці: DML відкочується разом із транзакцією, тому номер
повертається й **нумерація суцільна**; одночасні записи в одну область
серіалізуються до кінця транзакції. `sequence` — окрема послідовність на область:
не блокує паралельних писарів і не відкочується, тобто відкат лишає дірку
назавжди. Розривів не буває навіть від видалення — `delete` лише ПОЗНАЧАЄ запис,
тож виданий номер назавжди лишається за видимим рядком (через це не знадобилася
третя стратегія «номер при проведенні»). Перемикання стратегії на живому
нумераторі вимагає пересіву `app.numerator_seed_from_existing` — counter і
sequence ведуть лічильник у різних місцях і одне про одне не знають.

**Рік береться з дати документа, а не з `now()`**: документ, уведений заднім числом
у грудень, мусить отримати торішній лічильник. Зміна дати вже записаного документа
номер не змінює — але переносить документ в інший рік, тобто в іншу область
унікальності; якщо номер там зайнятий, запис відмовить, і це правильно (щоб узяти
номер нового року, поле очищають). **Виданий номер редагується**: порожнє поле означає «видай
наступний», ручне значення приймається як є, але підтягує лічильник — інакше через
кілька записів авто-номер упреться в уже зайнятий (перенумерація наявного запису
підтягує так само). Номер не за шаблоном лічильника не стосується взагалі. Ручний
номер можна заборонити зовсім — прапорець `is_editable` нумератора, і це серверна
відмова з прив'язкою до поля, а не лише вигляд форми.

Коди довідників поділені за природою, а не однаково: `currency.code` (ISO 4217) і
`chart_of_account.code` (ключ, на який посилаються проводки) лишаються ручними —
це самі дані; `counterparty`/`nomenclature`/`organization` нумеруються; у `bank`
окремого коду немає — ним є МФО. Показувати замість коду `id` не можна: його не
зарезервувати, не дати префікс і не зберегти при перенесенні рішення.

Правила живуть на екрані `app/admin/numerator/` (шаблон, період, стратегія,
заборона ручного номера, стан лічильників по областях): збереження там само
пересіває лічильники (`app.numerator_reseed` за джерелом, яке деплой кладе в
рядок нумератора), тож зміна стратегії чи шаблона не вимагає згадати про пересів.
Історія рішень і виконаний план — [`docs/numbering-plan.md`](docs/numbering-plan.md).
...
`app/<family>/<model>/_locales/<код>.json`, ті, що не належать жодній моделі
(`common.*`, `document.*`) — в `app/shared/_locales/`, оболонка — у своєму каталозі.
`app/_locales/*.json` — це **склейка** (`deno task locales:build`): вона комітиться,
бо саме її Vite копіює в `dist/locales/app/`, а правити її руками не можна — наступна
збірка зітре. Розсинхрон ловить проба в `test:unit`, а в самому файлі перший ключ —
`"//"` із попередженням (коментарів у JSON немає, а дивиться туди рівно той, хто
зібрався правити).

Доти файл був один на застосунок, і належність трималася на угоді про префікс
(`bank.*` — це модель `bank`). Угода вже розходилася з дійсністю, а перевірити її не
було чим: файл спільний, дописати можна що завгодно. Тепер належність структурна,
тобто збрехати про неї не можна, і **ключ із двома власниками — помилка збірки**, а не
тихе перевизначення. Ціна одна: рядок, потрібний двом моделям, треба свідомо винести в
`shared/` — і це видно в диффі.

Джерела `_locales/` НЕ можна класти всередину `app/_locales/`: пресет Vite копіює
його **плоско** (`stripBase: true`), тож підкаталог звалився б в одну теку з виходом.

**Мову називає застосунок.** `Locale` у фреймворку — вільний рядок, а не перелік:
щоб додати польську, досить покласти `app/_locales/pl.json` (тобто `pl.json` у
`_locales/` власників) — оголошувати її ніде не треба, версія пакета не потрібна.
Ключі, яких у цій мові немає, беруться з англійської (`FALLBACK_LOCALE`): порядок
накладання — `[запасна, обрана]`, усередині кожної спершу фреймворк, потім застосунок.
Тобто застосунок може перевизначити будь-який рядок фреймворку, але **мова сильніша за
перевизначення** — інакше при неповному перекладі в польському інтерфейсі проступали б
англійські кнопки там, де польський текст є. Файли застосунку тягнуться паралельно,
тож запасна мова не коштує другої затримки на старті.
...
хто відтворив саме ту відмову. Тому це під пробою: `scripts/translation-markers_test.ts`
сканує `server/` і `app/`, звіряє кожен `@[…]` зі словниками й вимагає обох мов.
Маркер у **коментарі** не рахується — там він цитата формату, а не вжиток
(`tools/scan-translation-markers.ts`, `stripComments`).

**У меню в базі лежить маркер, а не текст.** `app.menu_item.name` — це
`@[bank.titleMany]` чи `@[menu.group.catalog]`. Інакше меню лишалося б мовою
сіду, скільки б мов застосунок не знав — а це єдиний екран, який видно завжди.
...
**Друк маркера не знає й не мусить.** `printPdf` рендерить на сервері й повертає
готові байти: підписи лежать у шаблоні, а шаблон — дані в базі. Друкована форма в
бухгалтерії часто регламентована, тобто її не «перекладають», а мають окремо —
тож потрібна іншою мовою форма заводиться окремим шаблоном, як і на іншу модель.
Це єдине місце, де сервер справді рендерить текст, і воно свідомо лишається поза
цим домовленням.
...
**SQL звіту — це дві функції, і рукописна з них одна.** `sql:gen` будить зі схеми
фільтрів (`<Pascal>FiltersSchema`) обгортку `app.<report>_index` у
`db/_generated/<report>.index.gen.sql`: розбір `payload.filters`, відмова при
незаповненому обов'язковому фільтрі, эхо `$filters` з підписом із бази й конверт. Тіло —
`app.<report>_data(user_id, filters)` у `db/<report>.sql` — дістає вже розібрані значення
(ссылка згорнута до id: `organization` → `organizationId`) і віддає лише вміст `data`.
Обгортка нічого рукописного не заміняє, тому збирач бере **обидва** файли, на відміну від
CRUD, де генерація витісняє legacy-файл.

Походження ссылки оголошується в схемі фільтрів тим самим `x-ref`, що в моделях:
без нього підпис для эха брати нізвідки. Обов'язковість — це `Type.Optional`, як і в
полях форми; доти схеми звітів казали протилежне обом (організація стояла `Optional`
при `required` у розмітці, а період — обов'язковим), і це нічого не ламало рівно доти,
доки обов'язковість ніхто не читав.

Кожна з чотирьох дій обгортки окремо не варта уваги — і саме тому вони й розходилися:
методологію вхідного сальдо одного разу правили у двох звітах нарізно, і обидва однаково
вважалися джерелом правди.
...
**Часткове перенесення — інструмент розробника, а не поставка.** Скелет для подальшої
роботи, фактично копіювання моделей з місця на місце; працездатності на приймачі воно не
обіцяє й не мусить.
...
У пакет їдуть **рівно перелічені каталоги моделей**; завантаження **нічого не затирає** —
модель, яка в приймачі вже є, без `--force` не чіпається зовсім. Це протилежність повній
поставці, і саме тому вид пакета тримається на **номері формату** (повний — `1`, частковий
— `2`), а не на полі манифеста: старий приймач звіряє версію строгою рівністю, тож
розпакувати частковий як повний і викинути решту рішення він не може за побудовою.
`solution:update` частковий пакет не бере взагалі — оновлення установки означає поставку.

Три наслідки, які варто знати наперед: `app/sql.json` **дописується** (він належить
рішенню, і без запису модель не потрапить у зібраний SQL); установка сама переходить у
ручний режим, бо манифест поставки не переписується; **пункти меню не їдуть** — склад меню
це сід чужої моделі. Про посилання на те, чого в пакеті немає (чужа модель, `@shared/…`),
вивантаження **попереджає, але нічого не добирає**: набір моделей і є вибір людини.
