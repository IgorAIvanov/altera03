...
mcp/                        # MCP-обгортка над HTTP-API (@altera/mcp)
  main.ts                   # три інструменти + stdio: altera_models / describe / call
...
**Регістр читають одним шаром — `@core/ledger`** (`acc_entries`, `acc_journal`,
`acc_balance_turnover`, `acc_balance`, `acc_turnover`, `acc_account_tree`); підсумків
система не зберігає свідомо. До свого запиту варто знати: **що вважається рухом** сказано
один раз — в `acc_entries`; **вхідне сальдо з відкритою датою початку — нуль** (тому звіти
...
грішми, і **порожньо ≠ нуль** (порожньо — рахунок виміру не веде); розріз поки один
(`p_dimension_code`). Еталони — три звіти в `app/report/`.
