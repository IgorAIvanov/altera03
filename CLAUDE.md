...
читався б як «поміряли й вийшло нічого». **Кількість належить тому боку, чий рахунок її
веде**: колонка в регістрі одна й заповнюється, коли кількісний хоч один бік, тож у «Дт 281
Кт 631 на 10 шт» другій стороні діставалися ті самі 10 штук — тепер `acc_entries` гасить їх
за ознакою рахунку. І **розріз усе ще один** (`p_dimension_code`), тобто «скільки чого й ДЕ»
через шар не питається: запас живе у двох вимірах, склад × номенклатура.
