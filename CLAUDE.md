...
Головна прогалина не там, де здається: опис інструмента віддає `item` як
`{type: "object"}` — складу полів, типів і обов'язковості агент не бачить.
Схеми при цьому є (TypeBox у моделях), вони просто не доїжджають до сервера.

Рішення, порядок робіт і те, що свідомо відкладено, —
[`docs/external-agent-plan.md`](docs/external-agent-plan.md); коду ще немає.
