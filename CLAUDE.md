...
payload не зберігається. **Виклик персональним токеном — нижня межа, а не рівень:**
пишеться завжди (читання, запис, відмови, канал байтів), з `access_token_id`, і в тій
самій транзакції, що й команда — не записався рядок, відкочується дія. Відповідає за
агента людина, яка кроків не бачила; журнал — її спосіб дізнатися. Новий вхід, що
приймає токен, виконує дію через `withTokenAudit` (`server/common/token-audit.ts`);
TS-хендлер під токеном ходить у базу лише через `ctx.db` (він прив'язаний до
транзакції) — [`docs/agent-audit-plan.md`](docs/agent-audit-plan.md).
...
користувач не кандидат на знищення. Документи додатково: `post`, `unpost` — і `postPreview`,
**сухий прогін**: кличе ту саму `<model>_post` у транзакції, віддає проводки з субконто й
відкочує все, що вона написала. Пишеться він не в застосунку — ядро додає його кожному
документу само (`runtime.postPreview`), як `printPdf` моделям із друком. Дві межі: право
просить як у проведення (питати «що буде, якщо проведу» має сенс тому, хто проводить), але
запису не робить — тому його виконує й токен «тільки читання», не питаючи `confirm`, і в
журнал змін він не пише. Так само кожен документ має `related` — **дерево пов'язаних
документів** (`app.document_related`, право `view`): ребра — `app.document_link`, яке
`sql:assemble` виводить зі схем (кожен `x-ref` документа на документ, відмова —
`related: false`), права рахуються по вузлу; на формі — кнопка `<ui-related-documents>`
(маршрут вузла дає застосунок властивістю `routeOf`). Маршрут веде рантайм, не реєстр — план і межі
...
Команда, що йде хвилинами (перенесення даних, закриття місяця, перепроведення
періоду), не живе всередині HTTP-запиту. Модель оголошує її в манифесті —
`"commands": { "long": ["load"] }`, — і рантайм кладе роботу в `app.job` (пакет
ядра `@core/job`), одразу віддає завдання, а доробляє поза запитом. Стан питають
командою `job/get`; у `result` лежить **конверт самої команди**, тож клієнтського
коду на два випадки не потрібно.

Головне, чого не видно з коду: **довгою є команда, а не модель** (`load` і `get`
живуть в одній моделі), і оголошення лежить у манифесті навмисно — довгою
команду робить розмір бази, а не реалізація. **Одне активне завдання на пару
(модель, команда)** тримає унікальний індекс, а не перевірка перед вставкою:
перевірити й вставити — гонка, і програє вона там, де найдорожче. **Прогрес
пишеться іншим з'єднанням** (`ctx.job?.progress(...)`), бо записане всередині
транзакції видно лише після коміту. **Зняття — прохання**: команда зупиняється
сама, між кроками. **Покинуте завдання прибирає сторож** за відсутністю стуку —
інакше одне падіння сервера блокувало б команду назавжди. **На Deno Deploy фон
відмовляється** (`jobs.background` з `configFromEnv`): платформа не обіцяє життя
процесу після відповіді, а обірвана на середині робота гірша за чесну відмову.
Панель Cron там не рятує — не технічно, а грошима: щохвилинна крона не дає
застосунку заснути й сама з'їдає майже вдвічі більше пам'яті-часу, ніж увесь
безплатний тариф; рахунок і те, що на Deploy працює насправді, — у документі.
Транзакцією прогону ядро не керує — її межі обирає команда, і одна транзакція на
весь прогін тут не рекомендація, а пастка. Деталі —
[`docs/long-command.md`](docs/long-command.md).
...
`@[ключ]{...}`, розгортає клієнт; рядок без маркера не чіпається (діагностика).
Виняток один і межа в ньому не про важливість повідомлення: **сервер називає
ключ усім, крім того, кому нема кому його розгорнути** — тобто в каналі
зовнішнього агента (`/api/agent/call`) розгортає сам сервер, бо браузера там
немає. Словники він при цьому не шукає, а отримує від застосунку
(`bootstrap({ messages })`): рядки ядра везе `@altera/client`, рядки моделей —
`locales:build`, і обидва є тільки в композиційному корені. Ключі ядра
...
**Гарячі клавіші, посилання на вкладку, документ за кодом** — [`docs/ui-shell.md`](docs/ui-shell.md).
...
Штрихкод бланка несе id документа; `<ui-document-open>` у шапці віддає його команді ядра
`document.locate`, і маршрут форми рахує СЕРВЕР з view-manifest — `routeOf` тут не
потрібен.
...
банк-клієнт): вона ходить лише назовні, везе сотні тисяч рядків, і з'єднання
рветься. Пакет ядра `@core/import` — **схема під це**: сесія-канал, партії з
ідемпотентними частинами, сировина й карта зовнішніх ключів. Модель
`import_session` дає людині `list`/`get`/`close`/`volume`/`purge`; модель
`source_decision` — рішення про об'єкти джерела: агент **пропонує** пачкою
(`propose`), а `confirm`/`delete` виконує **лише людина** — токену їх відбиває
рантайм (`HUMAN_ONLY_COMMANDS`) незалежно від прав і `confirm`.

Головне, що з таблиць не видно: **сировина лягає окремо, бо конвертувати
доводиться багато разів** (ітерацій переносу п'ять-десять, і кожна не має
питати джерело заново); **сировини дві таблиці** — в рядка залишків власного
ключа немає, а таких рядків половина; **ключ включає партію**, інакше
простежуваність зникає на другому знімку; **рішення людини первинні, карта
похідна**, і обидва переживають знесення сировини; **автоочищення немає** —
зносить людина командою, і `purge` без названої області відмовляє. Закриття
сесії **відкликає токен** адаптера, а не малює напис. Токен каналу — звичайний
...
`GET /api/agent/tools` (каталог без схем; з `?model=…` — схеми payload-ів із TypeBox
плюс **оголошені обмеження** названих моделей: перелік `{key, text}` того, чого
застосунок робити не стане. Прив'язку «який ключ чия модель називає» збирає
`sql:registry` тим самим сканером маркерів, що й проба перекладів; правила ядра
додає `core:sql`, і ключуються вони не моделлю, а тим, кому стосуються —
`document` дістає все, що відбиває проведення; текст береться в рантаймі зі
словників повідомлень, тож перекладу ніде не дублюється) і
...
`commands.access`, вивід з імені знає лише стандартні дії). **Склад переліку називає
модель — `agent.allowCommands`, і він ДОДАЄ, а не лише відрізає**: умовчання дає набір
за типом (admin-моделі — `list`/`get`, тільки читання: там лежать відповіді на «чому
номер такий», а запис має надто велике плече) плюс друк, а рядок у списку тягне в перелік і оголошену команду
(`commands.sql`/`commands.ts`) — саме нею агент дістає `item_account.at` чи
`nomenclature.groupTree`. Дві межі: оголошення права в `commands.access` саме собою
агенту нічого не відкриває (`allowCommands` — окрема заява, і без неї модель лишається
на умовчанні), а ім'я, якого модель ніде не оголосила, тепер валить `sql:registry` —
доти воно зникало мовчки, і манифест виглядав так, ніби команду відкрито.
**Персональний токен**
...
із обмеженнями. Третій вид — **тема** (`kind = 'topic'`, «Порядок закриття
місяця»): процедура на сторінку-другу, тож завжди їде лише покажчик (ім'я,
назва, «коли потрібна»), а тіло агент бере командою `agent_note.topic` —
як у скіла з його frontmatter і тілом. У контекст іде лише підтверджене людиною; агент пропонує
...
бо пакет везе субсети Roboto та PT Sans під OFL-1.1 усередині себе. Поле
`license` при цьому лишається `MIT`, і не з недбалості: **JSR приймає один
упізнаваний ідентифікатор SPDX, а складений вираз відхиляє** — причому вже при
публікації, бо `deno publish --dry-run` ліцензію не перевіряє. Тобто твердження
про OFL живе там, де його не відхилить реєстр і де його справді читають, — у
нотисах поруч із байтами.
