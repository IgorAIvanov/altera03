...
Тому в `server/modules/agent/` **немає жодного LLM**. `agent.service.ts` —
диспетчер: звіряє модель і команду з білими списками (`manifest.agent`) і кличе
рантайм від імені користувача. Найважче для такої задачі зроблено не заради неї:
