...
Помилковий клік на проведеному документі більше не забирає з собою РЯДКИ документа, а
...
**Позначка на документі = скасування проведення.** Позначений документ не тримає
проводок: `delete` проведеного спершу знімає проведення (з гаком `_unpost_records`
включно), потім ставить позначку — і хук `doc_before_write` бачить ОБИДВІ події, тож
заборона закритого періоду діє й на видалення проведеного. Зняття позначки проведення НЕ
повертає: документ стає непроведеним, і провести його треба заново, свідомо. Інакше
«правду» про позначений документ виражав би фільтр у читанні — проводки лежать, звіти їх
не бачать, а «Відновити» мовчки повертало б рухи в баланс. Документи, позначені ще за
старим правилом, приводить до інваріанта міграція; ті, що прослизнули повз неї, доліковує
`undelete`.
...
**Регістр читають одним шаром — `@core/ledger`.** Підсумків система не зберігає свідомо, тож
сальдо й обороти рахуються скануванням `app.journal_entry`; шар дає `acc_entries` (потік рухів
ВІД РАХУНКУ — рядок це бік проводки), `acc_journal` (рядок це проводка), `acc_balance_turnover`
(сальдо й обороти за один прохід), `acc_balance`, `acc_turnover`, `acc_account_tree`. Він у ядрі,
бо регістр — таблиця ядра: хто володіє схемою руху, володіє й способом його прочитати. Два
правила, які варто знати до того, як писати свій запит: **що вважається рухом** (проведений,
непозначений, тієї організації) сказано ОДИН раз — в `acc_entries`; і **вхідне сальдо з відкритою
датою початку дорівнює нулю** — рух не може бути одночасно залишком і оборотом, тому звіти беруть
його з `acc_balance_turnover`, а не з `acc_balance(org, date_from)`. Подання (дебетове/кредитове
сальдо) шар не робить навмисно: він віддає чисте `net`, а розкладає його звіт — це різні речі,
арифметика й форма. Еталони — три звіти в `app/report/`, у яких після переходу власної арифметики
не лишилося.

**Кількість шар віддає разом із грішми** — `opening_quantity`, `quantity_debit`,
`quantity_credit`, `closing_quantity` в обох функціях сальдо-й-оборотів. Три речі, які з
цього ростуть. **Порожньо ≠ нуль**: порожньо там, де рахунок кількості не веде
(`is_quantitative`), нуль — там, де веде, але руху не було; нуль на негрошовому рахунку
читався б як «поміряли й вийшло нічого». **Кількість і валюта зберігаються по боках проводки**
(`quantity_debit`/`quantity_credit`, `currency_id_debit`+`currency_amount_debit` /
`currency_id_credit`+`currency_amount_credit` у `app.journal_entry`) — це небалансові
виміри, як у джерелі моделі; балансова лише сума, бо це і є подвійний запис. Комплектація
списує 6 корпусів і оприбутковує 2 комплекти однією кореспонденцією; конвертація
«Дт 312 USD Кт 314 EUR» несе дві різні валюти — нерівність і різність тут зміст операції,
а не помилка. Складна проводка — це кілька таких рядків, кількість головного боку пишеться
один раз; окремого механізму групи немає навмисно (сверяти в ній нема чого — сума голови
дорівнює сумі частин за побудовою). У `doc_entry_add` два шляхи для кожного виміру:
legacy-параметри (`p_quantity`, `p_currency_id`+`p_currency_amount`) — одне значення на всі
боки, що ведуть вимір, строго, як було; `p_quantities` (`{"debit": 2, "credit": 6}`) і
`p_currencies` (`{"debit": {"id": …, "amount": …}}`) — по боках, і перелік тут ВИСЛОВЛЮЄ
НАМІР: відсутній у ньому бік лишається порожнім законно, а `{}` — вимір без значення взагалі
(переоцінка). Половина валютної пари (`id` без `amount`) — помилка, а не намір. Обхід
конвертації через рахунки «в дорозі» (333/334) лишається законним, але тепер він вибір, а
не єдиний спосіб. І **розріз усе ще один** (`p_dimension_code`), тобто «скільки чого й
ДЕ» через шар не питається: запас живе у двох вимірах, склад × номенклатура.

**Проводка може мати порожній бік — і лише на забалансовому рахунку.** Забалансовий
облік однобічний за визначенням («Дт 021» не кореспондує ні з чим), а парна проводка через
допоміжний рахунок псує дані: у регістрі з'являється кореспонденція, якої в обліку немає.
Обидві колонки `app.journal_entry` нульові, зводить їх `ck_journal_entry_sides` (хоча б один
бік), а `doc_entry_add` вимагає `is_off_balance` — на балансовому рахунку порожній бік це
недописаний рядок. Наслідок для звітів: `union all` двох боків мусить відсікати `null`,
інакше порожній бік збереться в окремий рахунок і зробить підсумки «зведеними»; забалансові
рахунки в оборотці лишаються рядком, але з підсумку вилучені.

**Перед кожним записом шапки документа ядро кличе гак застосунку** —
`app.doc_before_write(user_id, op, doc jsonb, prev jsonb)`, з тригера на
`app.document`. Вмикається створенням функції, немає її — немає й перевірки;
функція з іншим підписом валить запис (мовчазний пропуск гірший за відсутність:
застосунок вважав би, що заборона діє). `op` називає дію словом застосунку
(`insert`/`update`/`post`/`unpost`/`delete`/`undelete`/`purge`) — з `TG_OP` це не
видно, там і проведення, і позначка на видалення однаково `update`. Тригером, а
не домовленістю «клич перевірку в кожній команді»: шляхів запису багато, і діяла б
вона там, де не забули. Сюди лягає заборона закритого періоду; сторожа на
`app.document` застосунок більше не ставить.
...
`{ key, titleKey, section? , content? }`, з секцією компонент малює все сам, `content` —
для вкладки зі звичайними полями. Два місця, через які це в ядрі, а не в застосунку.
**Помилка в СХОВАНІЙ таблиці**: `BaseUI` перевіряє всі секції з `sections()`, тож банер
скаже «Рядок 2, «Сума»…», коли рядка на екрані немає — вкладку вмикає сам компонент, і
за сигналом `pendingFocus` (одноразова подія від бази), а не за `errorCount`: слідкування
за станом тягло б користувача назад щоразу, коли він пішов подивитися сусідню частину.
Правила вибору лежать без DOM — `form-tabs-state.ts`, під пробами. **І вкладка знову
`<button>`**: у розмітці ФОРМИ смуга лежить усередині `fieldset[disabled]` з
`renderFields()`, каскад гасить усі нативні контроли, і проведений документ переставав
гортатися; каскад не перетинає межу тіньового кореня, тож усередині компонента кнопка
працює. Вигляд — `.form-tab*` у темі.

**У режимі перегляду контрол лишається й гасне, а не зникає** — і панель дій секції, і
колонка кошика в рядку. Зникнення читається як поламка («дії тут немає ніколи»), а
проведення форму не перезавантажує, тож смуга з'являлася й зникала під курсором, зсуваючи
таблицю. Прибрати їх і далі можна — не поставивши `<ui-tabular-toolbar>` (він незалежний)
або `rowDelete: false`. Випадок «реквізит правлять без розпроведення», тобто живий контрол
на проведеному документі, цим правилом не покривається — це окремий механізм, якого ще немає.
...
**Ліва панель меню** — дані, а не код: склад задає адміністратор на екрані
`app/admin/menu/`, оболонка (`app/menu/app-menu.ts`) його лише малює. Дві речі, які
видно не одразу. **Порожня тека не показується взагалі** — `menu_current` лишає теку,
тільки якщо під нею вцілів хоч один лист: порожня гірша за відсутню, бо в неї клікають
(звідси й «щойно створена тека не з'явилася» — це не поломка). І **коренева тека з кодом
`bottom` — не розділ, а нижня частина**: її вміст малюється закріпленим під списком, що
прокручується, сама тека на екран не йде. Домовленістю, а не колонкою в `app.menu_item`:
склад меню — справа застосунку, а колонка означала б міграцію ЯДРА й нове поле в редакторі
заради того, що адміністратор виражає структурою, яку й так редагує. Деталі, пастки й
порядок дій у редакторі — [`docs/left-menu.md`](docs/left-menu.md).
...
Працює **готовий зовнішній агент від імені реального користувача**: своєї
«розумної» частини ми не пишемо — агент, здатний тримати контекст і вирішувати,
це чужий продукт, і догнати його неможливо. Наша роль вужча: чесно описати, що
можна робити, кому й у яких межах.

Тому в `server/modules/agent/` **немає жодного LLM**. Два входи, і читаються вони
як у MCP: `GET /api/agent/tools` — перелік інструментів, `POST /api/agent/call` —
виклик інструмента (`{model, command, payload}`). `agent.service.ts` —
диспетчер: звіряє модель і команду з тим самим переліком і кличе рантайм від
імені користувача. **Білий список тут не свій** — це рівно те, що агент прочитав
у `tools`, тобто зібране з манифестів (`agent.allow`, `allowCommands`): другий
перелік розійшовся б із першим мовчки, і саме через це звіт не викликався б —
`tools` показував би `index`, а диспетчер відмовляв. Найважче для такої задачі
зроблено не заради неї:
право рахується на кожній команді (fail-closed), `Authorization: Bearer` уже
приймається, аудит пише виконавця, а `manifest.agent` дає другі ворота, вужчі за
права.

**Персональний токен** (`app.access_token`, `@core/access`) — облікові дані для
машини: `Authorization: Bearer`, права не несе (їх рахує `access_can` як
завжди), видається ЛИШЕ з браузера по сесії, у базі лежить хеш. Керування — на
`/api/auth/tokens`, поза правами моделей: токен це «моє», як власний пароль, а
модель вимагала б прав на неї в кожного користувача.

**Опис інструментів — `GET /api/agent/tools`, і він у двох режимах.** Без
параметрів — **каталог**: модель, тип, назва мовами застосунку (`titles`),
маршрут, `aliases`, перелік команд, без схем. З `?model=bank,invoice` (і, за потреби, `&command=save`) — **схеми**: `{model,
command, input}`, де `input` це JSON Schema payload-а, згенерована з TypeBox-схем
моделей (`tools/agent-tool-schemas.ts` → `app/_generated/agent-tools.generated.ts`,
передається в `bootstrap()` полем `agentTools`). Доти агент бачив `item` як
`{type: "object"}` і не знав ані складу полів, ані типів.

Поділ — арифметика, а не смак: 20 моделей це 65 інструментів і 26 КБ, сотня
моделей — 400–600 інструментів і ~200 КБ **в одній відповіді**. Каталог росте
інакше — заміряно 260 байтів на модель разом із назвами й синонімами (на
прикладному рішенні: 67 моделей, 17 КБ), — тож його тримають цілком, а схему
беруть тоді, коли справді збираються викликати. Перелік **фільтрується правами
користувача** в обох режимах — не як захист (він нижче, у рантаймі), а щоб агент
не витрачав крок на команду, яка йому відмовить.

**Звіти агенту доступні** — команда `index` зі схемою фільтрів (оборотка, картка
рахунку, рухи документа). Це не повнота заради повноти: агентська робота
починається з того, щоб ПОДИВИТИСЯ, і без звітів набір був рівно протилежний до
потрібного — створити й провести документ можна, звіритися ні. Право такої
команди береться **з оголошеного** (`commands.access`), а не виводиться з імені:
вивід знає лише вісім стандартних дій, тож усе нестандартне — звіт, друк,
TS-команда — було невидиме, хоч і виконуване.

**Запобіжники запису діють лише на виклики токеном**: `delete`/`undelete`/
`post`/`unpost` вимагають `"confirm": true`, а токен `isReadOnly` відсікає запис
незалежно від прав і не показує інструментів запису взагалі. Людина з браузера
підтвердила натисканням кнопки — вимагати від неї ще й прапорця означало б
зламати кожну форму; агент мусить сказати це словом. Стоїть це в
`ModelRuntimeService.execute`, у тій самій точці, де рахується право, тож обійти
іншим входом не виходить.

**Вбудований LLM-клієнт видалено** (server 0.18.0): `agent-llm.service.ts`
(OpenAI Responses API з власним роутером і tool-циклом), ендпойнт
`POST /api/agent/llm-chat` і `OPENAI_*` у конфігурації. Його не кликав ніхто —
ні екран, ні клієнтський код, — а їхав він у кожен опублікований пакет разом із
ключем у налаштуваннях. Помічник усередині застосунку («проведи цей рахунок» у
чаті на екрані) цим не закритий, але це інша задача: вона вимагає LLM на
СЕРВЕРІ, тобто рівно того, що тут прибрано.

**MCP-обгортка** — окремий пакет `mcp/` (`@altera/mcp`), не частина сервера: MCP
ще рухається, і версія протоколу мусить бути клопотом обгортки. Усередині —
офіційний TS SDK, три інструменти (`altera_models` / `altera_describe` /
`altera_call`) замість дзеркала команд: опис інструментів у MCP лежить у
контексті ЗАВЖДИ, тож дзеркало на сотню моделей коштувало б ~200 КБ у кожній
розмові. **Одна база на процес** — кілька баз означає кілька записів у конфізі
хоста; параметр «до якої бази» був би зручніший, але промах у ньому пише не в ту
базу. Деталі й запис у конфіг — [`mcp/README.md`](mcp/README.md); проба
(`deno task mcp:test`) піднімає підроблену базу й розмовляє з обгорткою
справжнім JSON-RPC.

Рішення, порядок робіт і те, що свідомо відкладено, —
...
(свій git — **локальний, без remote**: коміт є частина правки, пушити нікуди):
