...
begin
  if exists (select 1 from information_schema.columns
             where table_schema = 'app' and table_name = 'counterparty' and column_name = 'is_active')
  then
    alter table app.counterparty add column if not exists is_deleted boolean not null default false;
    update app.counterparty set is_deleted = not is_active;
    drop index if exists app.ix_counterparty_is_active;
    alter table app.counterparty drop column is_active;
  end if;
end $$;
...
create index if not exists ix_counterparty_is_deleted on app.counterparty (is_deleted);
