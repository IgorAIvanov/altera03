...
begin
  if exists (select 1 from information_schema.columns
             where table_schema = 'app' and table_name = 'nomenclature' and column_name = 'is_active')
  then
    alter table app.nomenclature add column if not exists is_deleted boolean not null default false;
    update app.nomenclature set is_deleted = not is_active;
    drop index if exists app.ix_nomenclature_is_active;
    alter table app.nomenclature drop column is_active;
  end if;
end $$;
...
create index if not exists ix_nomenclature_is_deleted on app.nomenclature (is_deleted);
