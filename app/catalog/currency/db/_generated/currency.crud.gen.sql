...
-- Джерело: currency.schema.ts + manifest.json. Override — db/currency.custom.sql


drop function if exists app.currency_list(bigint, jsonb);
create function app.currency_list(user_id bigint, payload jsonb)
returns jsonb
language plpgsql
as $$
declare
...
drop function if exists app.currency_get(bigint, jsonb);
create function app.currency_get(user_id bigint, payload jsonb)
returns jsonb
language sql
as $$
  select jsonb_build_object(
...
drop function if exists app.currency_save(bigint, jsonb);
create function app.currency_save(user_id bigint, payload jsonb)
returns jsonb
language plpgsql
as $$
declare
...
begin
  ...
  using (
    select
      ...
      nullif(trim(coalesce(v_item->>'numericCode', '')), '') as numeric_code,
      nullif(trim(coalesce(v_item->>'symbol', '')), '') as symbol,
  ...
  when matched then update set
    code = s.code,
    name = s.name,
    numeric_code = s.numeric_code,
    symbol = s.symbol,
    is_deleted = coalesce(s.is_deleted, t.is_deleted),
    updated_at = now()
  when not matched then insert (code, name, numeric_code, symbol, is_deleted)
    values (s.code, s.name, s.numeric_code, s.symbol, coalesce(s.is_deleted, false))
...
drop function if exists app.currency_delete(bigint, jsonb);
create function app.currency_delete(user_id bigint, payload jsonb)
returns jsonb
language plpgsql
as $$
declare
...
begin
  ...
  update app.currency set is_deleted = true where id = v_id;
...
drop function if exists app.currency_undelete(bigint, jsonb);
create function app.currency_undelete(user_id bigint, payload jsonb)
returns jsonb
language plpgsql
as $$
declare
...
begin
  ...
  update app.currency set is_deleted = false where id = v_id;
...
drop function if exists app.currency_lookup(bigint, jsonb);
create function app.currency_lookup(user_id bigint, payload jsonb)
returns jsonb
language plpgsql
as $$
declare
