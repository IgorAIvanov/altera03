...
deno task solution:import -- <пакет.tar.gz> [--check] [--force]   # завантажити його сюди
```
...
Завантаження заміняє `app/` **цілком** (тому непорожній каталог вимагає
`--force`): домовленість — змінене на приймачі рішення не зливається. Далі —
штатний ланцюжок, який інструмент навмисно не запускає сам:

```bash
deno install && deno task sql:registry && deno task sql:assemble && deno task sql:publish && deno task build:front
```
