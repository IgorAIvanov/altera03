...
function agentBaseCommands(type: string): string[] {
  ...
  // Admin-екран — ТІЛЬКИ читання, і це не половинчастість.
  //
  // Нумератор, налаштування, шаблон друку, рівень журналу — саме там лежать
  // відповіді на «чому номер такий», «чому не друкується», «чому цього немає в
  // журналі». Доти агент їх не бачив узагалі, і питання лишалися без відповіді
  // не тому, що бракувало права, а тому, що моделі для нього не існувало.
  //
  // Читання, бо запис тут має інше плече: неправильний нумератор чи облікова
  // політика не падають, а тихо викривляють усе, що порахують далі. Той самий
  // довід, з якого ми не проводимо документи без людини.
  //
  // І це НЕ видача доступу: перелік однаково фільтрується правами користувача,
  // від імені якого працює агент (`permittedTools`). Тобто ховати команду від
  // того, кому адміністратор право вже дав, — не захист, а неточність опису.
  // Застосунок, який вважає інакше, закриває модель `"agent": { "allow": false }`.
  if (type === "admin") return ["list", "get"];
  return [];
}
