 ...
 *
 * **Один фільтр — один ключ, і для ссылки теж.** Значення ссылочного фільтра —
 * ОБ'ЄКТ `{ id, <display> }`: id вибирає записи, представлення показує пікер у
 * панелі. У SQL з нього береться сам лише `id`, а назад під тим самим ключем
 * приходить об'єкт, зібраний з того id і підпису з бази, — відповідь уточнює
 * фільтр, а не додає до нього другий.
 *
 * Пара «`counterpartyId` + окремий `counterparty`» виглядала природною й
 * коштувала двох тихих вад: значок на кнопці «Фільтри» рахував підпис за другий
 * фільтр (один обраний контрагент — «2»), а при скиданні id підпис лишався в
 * наборі назавжди — клієнт слав його назад незміненим, а сервер незмінним і
 * повертав. Одним ключем обидві не виникають узагалі: «скільки фільтрів діє» і
 * «що слати» лишаються просто вмістом `$filters`.
 */
...
export const listRootSchema: TObject<{
  ...
  // те, що це пари «ключ payload → значення»; значення буває й об'єктом (ссылка).
