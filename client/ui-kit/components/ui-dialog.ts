...
export class UiDialog extends GlobalStyledLitElement {
   ...
   *
   * Подія означає «людина попросила закрити», а не «вікно закрилося»: коли
   * власник гасить `open` сам (згорнув вікно, надіслав форму), повертати йому
   * його ж дію означало б, що він скасовує сам себе — і чернетка зникала б від
   * згортання. Відрізняємо за станом: у власній дії `open` уже `false`.
   * Прапорцем це не зробити — подію `close` браузер шле не синхронно.
   */
  #close = () => {
    if (!this.open) return;
