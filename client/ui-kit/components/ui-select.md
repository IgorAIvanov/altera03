...
|---|---|
| `↑` / `↓` | нативний вибір |
| `Enter` | перехід до **наступного контрола** — і з порожнім значенням. Список НЕ розкриває |
| `F4` / `Alt+↓` | розкрити список (нативно) |
| `Delete` | очистити (лише з `show-clear`) |
| `Tab` / `Shift+Tab` | наступний / попередній контрол |

**`Enter` список не розкриває.** Chrome на Windows розкриває `<select>` по
`Enter` — на `keypress`, тобто вже після того, як поле віддало фокус далі:
фокус ішов до наступного контрола, а список цього розкривався позаду. Тому
`keypress` Enter компонент гасить; розкривають список `F4` або `Alt+↓`.

**Кнопка очищення — поза Tab-чергою** (`tabindex="-1"`): `Tab` веде до
наступного поля, а не на хрестик цього ж. З клавіатури її заміняє `Delete`.
