...
export interface SubordinateConfig<Row extends object> {
  ...
  dateField?: string;
  /**
   * ОСНОВА ключів відбору за датою, якщо вона не збігається з `dateField`.
   *
   * Панель шле `<основа>From` / `<основа>To`, і доти основою було саме ім'я
   * поля. Але `x-filter` уміє перейменувати відбір — `{ "op": "range", "key":
   * "date" }` на полі `period` дає згенерованому `_list` ключі `dateFrom` /
   * `dateTo`, — і форма ця не екзотична: саме її несе `DocumentHeaderSchema`
   * фреймворку, бо такі імена віддає `<ui-period>`. Тобто два механізми ядра
   * вимагали від одного відбору різних імен, а мирив їх застосунок, знімаючи
   * перейменування зі схеми.
   *
   * Значення — та сама ОСНОВА, що в `x-filter.key` (`"date"`), а не готовий
   * ключ: відбір за датою це пара, і одним рядком її не назвати.
   *
   * Промах тут не падає, а тихо бреше: невідомий ключ згенерований `_list`
   * ПРОПУСКАЄ, тож лічильник «скільки рядків по той бік дати» повертає всі
   * рядки, перехід стає на першу сторінку — і виглядає рівно так, ніби він
   * відпрацював. Тому обидві сторони називають ключ явно, а не вгадують.
   */
  ...
  titleKey?: string;
  /**
   * Текст порожнього переліку — ключ локалізації; умовчання `common.noData`.
   *
   * Порожній ПЕРІОДИЧНИЙ регістр — це не «нічого немає», а «діє умовчання», і
   * умовчання в кожного своє: без курсів документи візьмуть 1:1, без облікової
   * політики — загальна система й 20 % ПДВ, без цін ціна в документ не
   * підставиться. Це відповідь на питання, яке людина ставить, побачивши
   * порожню таблицю, — і знає її лише застосунок: вона в його ж функціях `_at`
   * і в проведенні. Тому ядро тут може дати тільки місце.
   */
  emptyKey?: string;
  /**
...
export class SubordinateRegister<Row extends object> {
   ...
   * поля — під іменем поля або під тим, на яке його перейменували
   * (`dateFilterKey`).
   */
  get anchorFilterKey(): string {
    const base = this.config.dateFilterKey ?? this.config.dateField ?? "";
    return `${base}${this.descending ? "To" : "From"}`;
  }
