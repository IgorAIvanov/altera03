...
/**
 * Токен «тільки читання» спробував щось записати.
 *
 * ЧОМУ ЦЕ ОКРЕМА ПОМИЛКА, А НЕ ПЕРЕВІРКА В ОДНОМУ МІСЦІ. Запобіжник спершу
 * стояв лише в `ModelRuntimeService.execute`, і про нього було написано, що
 * «обійти іншим входом не вийде». Виявилося — виходить: байти вкладень ходять
 * НЕ командою моделі, а власним каналом (`POST /api/blob/upload`), і той про
 * прапорець не знав. Тобто токеном для читання можна було залити в базу
 * будь-який файл до ліміту розміру.
 *
 * Корінь помилки не в забутому рядку, а в тому, що обіцянка була про ТОКЕН, а
 * перевірка стояла в одного споживача. Тому вона тепер тут, поруч із рештою
 * помилок межі запиту: кожен новий вхід, який пише, має покликати
 * `assertTokenMayWrite` — і видно це буде з цього ж файлу, а не з чужого
 * сервісу. Друге правило того самого роду — журнал: кожен вхід, який приймає
 * токен, виконує дію через `withTokenAudit` (`token-audit.ts`).
 *
 * 403, а не 401: облікові дані дійсні, бракує саме права цього токена.
 */
export class ReadOnlyTokenError extends Error {
  readonly status = 403;

  constructor(what: string) {
    super(`Цей токен доступу — тільки для читання: ${what} заборонено.`);
    this.name = "ReadOnlyTokenError";
  }
}

/**
 * Відмовити, якщо запит прийшов токеном «тільки читання».
 *
 * `what` — те, що збиралися зробити, у знахідному відмінку («завантаження
 * файлу»): текст читає машина, тож він має називати дію, а не радити.
 */
export function assertTokenMayWrite(
  auth: { accessToken?: { readOnly: boolean } },
  what: string,
): void {
