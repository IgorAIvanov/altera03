...
--
-- Рядки, де цього виміру НЕМА, у результат не потрапляють — `cross join lateral`
-- відсікає їх сам. Так і треба: рахунок, який цього субконто не веде, не має
-- ані сальдо, ані оборотів «у розрізі нього», і показувати його порожнім рядком
-- означало б відповідати на не задане питання.
