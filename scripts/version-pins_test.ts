...
/**
 * Ліцензія кожного пакета — ОДИН ідентифікатор, який упізнає реєстр.
 *
 * JSR не приймає складених виразів SPDX: `"license": "MIT AND OFL-1.1"` — цілком
 * законне твердження за стандартом — валить публікацію словами «license … was
 * not recognized». Ціна помилки непропорційна її розміру: `deno publish
 * --dry-run` ліцензію не перевіряє ВЗАГАЛІ, `deno check`, `test:unit` і
 * `scaffold:verify` — тим паче, тож дізнаєшся про неї вже після пуша, від
 * червоного кроку публікації, коли номери версій уже витрачені.
 *
 * Саме так `@altera/server@0.25.3` не поїхав, а разом із ним не поїхало нічого:
 * цикл публікації йде за залежностями й падає на першому ж пакеті.
 *
 * Те, що пакет везе не лише свій код (субсети шрифтів під OFL у `server`),
 * лишається сказаним — але в THIRD-PARTY-NOTICES.md, який реєстр не читає й
 * відхилити не може.
 */
Deno.test("ліцензія пакета — один ідентифікатор, а не вираз SPDX", async () => {
  const problems: string[] = [];

  for (const name of [...PACKAGES, "create"]) {
    const license = (await readJsonc(`${name}/deno.json`)).license;
    if (typeof license !== "string" || !/^[A-Za-z0-9.+-]+$/.test(license)) {
      problems.push(`${name}/deno.json: ${JSON.stringify(license)}`);
    }
  }
  ...
  assertEquals(
    problems,
    [],
    "реєстр прийме лише один упізнаваний ідентифікатор SPDX — " +
      "складений вираз валить публікацію після зеленого CI",
  );
});
